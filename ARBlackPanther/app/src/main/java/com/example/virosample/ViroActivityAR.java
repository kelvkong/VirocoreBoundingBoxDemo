...
public class ViroActivityAR extends Activity {
    ...
    private Node createDummyNode(Vector pos){
        Node dummyNode = new Node();
        dummyNode.setOpacity(0.25f);
        dummyNode.setVisible(false);
        dummyNode.setPosition(pos);

        Box redBox = new Box(1,1,1);
        Material redMaterial = new Material();
        redMaterial.setDiffuseColor(Color.RED);
        redBox.setMaterials(Arrays.asList(redMaterial));
        dummyNode.setGeometry(redBox);
    ...
    private void trackImageNodeTargets() {
        ...
        mScene.setListener(new ARScene.Listener() {
            ...
            public void onAnchorFound(ARAnchor anchor, ARNode arNode) {
                ...
                Log.e("Test", "(onAnchorFound) Found anchor ID [" + anchorId + "] v.s. image target ID [" + mImageTarget.getId() + "]");
                ...
                //Show bounding box
//                mBoundingBoxNode.setPosition(pos);
                showBoundingBox("onAnchorFound");
            ...
            public void onAnchorUpdated(ARAnchor anchor, ARNode arNode) {
                //No-op
                //String anchorId = anchor.getAnchorId();
                //Log.e("Test", "(onAnchorUpdated) Found anchor ID [" + anchorId + "] v.s. image target ID [" + mImageTarget.getId() + "]");

                //Show bounding box
                //showBoundingBox("onAnchorUpdated");
            }

            @Override
            public void onAnchorRemoved(ARAnchor anchor, ARNode arNode) {
                String anchorId = anchor.getAnchorId();
                Log.e("Test", "(onAnchorRemoved) Found anchor ID [" + anchorId + "] v.s. image target ID [" + mImageTarget.getId() + "]");
                if (!mImageTarget.getId().equalsIgnoreCase(anchorId)) {
                    return;
                }

                //Show bounding box
                showBoundingBox("onAnchorRemoved");
    ...
    private void showBoundingBox(final String name){
        //ADDED
    ...
    private void startPantherExperience() {
        ...
        Log.e("Test", "startPantherExperience");
        ...
        animationJump.setListener(new Animation.Listener() {
            ...
            public void onAnimationStart(Animation animation) {
                //No-op
                Log.e("Test", "startPantherExperience - onAnimationStart");
            }

            @Override
            public void onAnimationFinish(Animation animation, boolean canceled) {
                Log.e("Test", "startPantherExperience - onAnimationFinish");
